...
if strcmpi(equalization_method,'ZF')
    ...
    data_channel_gains = channel_gains(data_indecies);
    if strcmpi(imp_type,'Fixed')
        W = divide(numerictype(data_channel_gains),conj(data_channel_gains),((abs(data_channel_gains)).^2+(0/SNR)));
        for i = 1:48:length(data)
        equalized_data(i:i+47) = data(i:i+47).*W;
        end
    else
        W = conj(data_channel_gains)./((abs(data_channel_gains)).^2 + (0/SNR));
        for i = 1:48:length(data)
            equalized_data(i:i+47) = data(i:i+47).*W;
        end
    end
