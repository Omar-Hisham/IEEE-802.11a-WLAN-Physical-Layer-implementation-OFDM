function [decoded_data, rec_frames1, eq_rec_fremaes1 ] = Receiver(Frames, equalization_method, rep_type, snr,  SCRAMBLER_INIT_STATE, R)
...
while n < length(Frames)
    ...
    eq_rec_fremaes = equalize_channel(rec_frames,channel_gains,equalization_method, rep_type, mm, 1);
    ...
    % -------- De-Interleaving --------- %
    if ~NO_SCRAMBLE_FLAG
       out_data = deinterleaving(out_data, R);
    end 
